...
const int TrafficGraphData::NEODASHD_DATA_SAMPLES = 800;
const int TrafficGraphData::NeodashdQueueSizes[] = {
        TrafficGraphData::NEODASHD_DATA_SAMPLES,     //Range_5m
        TrafficGraphData::NEODASHD_DATA_SAMPLES/2,   //Range_10m
        TrafficGraphData::NEODASHD_DATA_SAMPLES*2/3, //Range_15m
        TrafficGraphData::NEODASHD_DATA_SAMPLES/2,   //Range_30m
        TrafficGraphData::NEODASHD_DATA_SAMPLES/2,   //Range_1h
        TrafficGraphData::NEODASHD_DATA_SAMPLES/2,   //Range_2h
        TrafficGraphData::NEODASHD_DATA_SAMPLES*2/3, //Range_3h
        TrafficGraphData::NEODASHD_DATA_SAMPLES/2,   //Range_6h
        TrafficGraphData::NEODASHD_DATA_SAMPLES/2,   //Range_12h
        TrafficGraphData::NEODASHD_DATA_SAMPLES/2,   //Range_24h
    } ;

const int TrafficGraphData::SMALLEST_SAMPLE_PERIOD =
        TrafficGraphData::RangeMinutes[TrafficGraphData::Range_5m] * 60 * 1000 / TrafficGraphData::NEODASHD_DATA_SAMPLES;
...
void TrafficGraphData::tryAddingSampleToStash(GraphRange range)
{
    ...
    if (queue.size() > NeodashdQueueSizes[range]){
           sampleStash[range].push_front(queue.at(NeodashdQueueSizes[range]));
    }
}
...
{
    SampleQueue& queue = sampleMap[range];
    if (queue.size() == NeodashdQueueSizes[range] + 2){
...
{
    SampleQueue result;
    int lastUnusedSample = std::min(rangeQueue.size()-1 ,NEODASHD_DATA_SAMPLES-1);
...
TrafficGraphData::SampleQueue TrafficGraphData::getCurrentRangeQueue()
{
    ...
    getRangeQueue(currentGraphRange).mid(0,NEODASHD_DATA_SAMPLES).swap(newQueue);
    return newQueue;
}
