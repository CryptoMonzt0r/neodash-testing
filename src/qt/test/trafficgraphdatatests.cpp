...
void TrafficGraphDataTests::simpleCurrentSampleQueueTests()
{
    ...
    for (int i = 0; i < TrafficGraphData::NEODASHD_DATA_SAMPLES; i++)
    ...
    QCOMPARE(queue.size(), TrafficGraphData::NEODASHD_DATA_SAMPLES);
    for (int i = 0; i < TrafficGraphData::NEODASHD_DATA_SAMPLES; i++){
        QCOMPARE((int)queue.at(i).in, TrafficGraphData::NEODASHD_DATA_SAMPLES - i - 1);
        QCOMPARE((int)queue.at(i).out, TrafficGraphData::NEODASHD_DATA_SAMPLES - i - 1);
    }
...
void testQueueFill(TrafficGraphData::GraphRange range, int multiplier)
{
    ...
    TrafficGraphData::SampleQueue queue = trafficGraphData.getCurrentRangeQueue();
    QCOMPARE(queue.size(), std::min(size / multiplier, TrafficGraphData::NEODASHD_DATA_SAMPLES));
...
{
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 2);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 3);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 6);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 12);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 24);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 36);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 72);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 144);
    testQueueFillAndCheckRangesForSize(TrafficGraphData::NEODASHD_DATA_SAMPLES * 288);
}
...
{
    TrafficGraphData trafficGraphData(TrafficGraphData::Range_5m);
    for (int i = 1; i <= TrafficGraphData::NEODASHD_DATA_SAMPLES; i++){
        trafficGraphData.update(TrafficSample(1, 1));
    }
    QCOMPARE(trafficGraphData.getCurrentRangeQueue().size(),TrafficGraphData::NEODASHD_DATA_SAMPLES);
...
void TrafficGraphDataTests::averageBandwidthTest()
{
    ...
    QCOMPARE(trafficGraphData.getCurrentRangeQueueWithAverageBandwidth().size(), TrafficGraphData::NEODASHD_DATA_SAMPLES);
    ...
    QCOMPARE(trafficGraphData.getCurrentRangeQueueWithAverageBandwidth().size(), TrafficGraphData::NEODASHD_DATA_SAMPLES / 2);
    for(auto& sample : trafficGraphData.getCurrentRangeQueueWithAverageBandwidth()){
        QCOMPARE(sample.in, 1.0f);
        QCOMPARE(sample.out, 1.0f);
    }
